...
import { getNivelBySlug, getNivelByOrden, NIVELES_SEED } from '@/src/lib/supabase'
...
export default function NivelPage() {
  ...
  const { patrulla, loading, advanceLevel } = session;
...
useEffect(() => {
    ...
    async function fetchNivel() {
      if (patrulla && !currentNivel && slug) {
        // Usamos 'await' porque ahora la función se conecta a la base de datos
        const { data: nivel, error } = await getNivelBySlug(slug)
        
        if (nivel && !error) {
          // Verificar que la patrulla tiene acceso a este nivel
          if (nivel.orden > patrulla.nivel_actual) {
            router.push('/')
            return
          }
    ...
    fetchNivel()
  }, [patrulla, loading, currentNivel, slug, router])
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (answer === currentNivel.url_respuesta) {
      ...
      setTimeout(() => {
        if (currentNivel.url_respuesta === '__FIN__') {
          ...
          if (nextNivel) {
            router.push(`/nivel/${nextNivel.url_slug}`)
          }
        }
      }, 1500)
    } else {
